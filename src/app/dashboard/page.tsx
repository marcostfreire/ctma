...
// Enrollment interface with course relation (Supabase returns array for joins)
    ...
    if (user) {
      const fetchDashboardData = async () => {
        ...
        try {          // Fetch User Profile
          ...
          });          // Fetch Enrollments with course names
          ...
          if (enrollmentsError) throw enrollmentsError;          setEnrollments(enrollmentsData?.map((e: EnrollmentFromSupabase) => {
            const courseName = e.courses && e.courses.length > 0 ? e.courses[0].name : 'Curso Desconhecido';
            return {
              id: e.id,
          ...
          const { data: donationsData, error: donationsError } = await supabase
            .from('donations')
            .select('id, amount, created_at, status, message, donor_name') 
            .eq('donor_email', user.email) // Use donor_email instead of user_id
            .order('created_at', { ascending: false });          if (donationsError) throw donationsError;

          setDonations(donationsData?.map((d: { id: string; amount: number; created_at: string; status: string; message?: string; donor_name?: string; }) => ({
            id: d.id,
            amount: d.amount,
            date: d.created_at,
            type: 'Doação', // Fixed type since type column doesn't exist
