  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        const response = await fetch('/api/submit-form', {
            ...
            body: JSON.stringify({ formData, formType: 'Especializações' }),
        });
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <form onSubmit={handleSubmit} className="bg-white text-gray-900 p-8 rounded-lg shadow-xl">
            ...
            <div className="text-center">              <button
                ...
                className="bg-orange-500 hover:bg-[#3E567B] text-white px-8 py-4 rounded-lg text-lg font-semibold transition-colors w-full md:w-auto disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? 'Enviando...' : 'Escolher Minha Especialização'}
              </button>
            </div>
          </form>
          {submitStatus && (
